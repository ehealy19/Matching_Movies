...
2. checked the variance inflation factor to check multicollinearity of variables (those above the threshold of 10 were excluded)
international excluded
other language excluded
musical genre excluded
accepted mpaa rating excluded
1. estimate the propensity scores using a logistic regression with the other characteristics as covariates and female_lead as the treatment
   1. this produces propensity scores (ps) for each Title which is the prob of movie having a female lead
2. then match the movies across the groups (males versus female) based on closest propensity score
3. create the matched dataset (each row is matched pair)
4. compare the average box office of matched female vs male lead movies
